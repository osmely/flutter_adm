...
public class ADMMD5ChecksumCalculator {
     ...
     * 
     * @param input
     *            The input to compute the MD5 checksum on.
     * @return The MD5 checksum of the input.
     */
    static public String calculateChecksum(final Map<String, String> input) {
        final String serializedMapData = getSerializedMap(input);
        final byte[] md5Bytes = getMd5Bytes(serializedMapData);
        final String base64Digest = new String(Base64.encode(md5Bytes,
                Base64.DEFAULT));
        return base64Digest;
    }
     ...
     */
    private static String getSerializedMap(final Map<String, String> input) {
        final SortedMap<String, String> sortedMap = new TreeMap<String, String>(
                new UTF8CodeUnitStringComparator());
     ...
     * 
     * @param input
     *            The string used to generate the md5 digest.
     * @return The bytes of the md5 result.
     */
    private static byte[] getMd5Bytes(final String input) {
    ...
    static private class UTF8CodeUnitStringComparator implements Comparator<String>,
            Serializable {
        ...
        public int compare(final String str1, final String str2) {
            try {
                ...
                // Calculate the loop bounds so that we do not exceed the length
                // of either string.
                ...
                for (int i = 0; i < loopBounds; i++) {
                    ...
                    // If the two code units are not equivalent, return the
                    // difference.
                    if (ub1 != ub2) {
                        return ub1 - ub2;
                    }
